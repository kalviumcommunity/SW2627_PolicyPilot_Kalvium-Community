...
class RAGPipelineService:
    ...
    def retrieve_context(
        ...
        Args:
            ...
            k: Maximum number of top chunks to return (default 4).
            chunks: Optional candidate chunk pool (defaults to self.chunks).

        Returns:
            List of top-k chunk dictionaries sorted by similarity score descending.
        """
        # Handle case where k and chunks might be passed positionally or flexibly
    ...
    def assemble_context(self, chunks: List[Dict[str, Any]]) -> str:
        ...
        Each chunk is formatted with a clear numerical citation label and source:
            [1] Source: <source>
            <text>
        ...
        Returns:
            Formatted context string containing all chunk texts and citations.
        """
        if not chunks:
            return ""
    ...
    def generate_answer(self, query: str, context: str) -> str:
        ...
        Args:
            ...
            context: Assembled context text string with citations.

        Returns:
            Answer string generated by LLM or offline fallback.
        """
        ...
        # Try parsing JSON structure returned by ResponseService
        try:
            parsed = json.loads(raw_answer)
            if isinstance(parsed, dict) and "answer" in parsed:
                answer_str = str(parsed["answer"])
                # Map standard out-of-scope message to clear fallback if needed
    ...
    def answer_query(
        ...
        Pipeline Flow:
            query -> embed_query -> retrieve_context -> assemble_context -> generate_answer -> answer + sources

        Args:
            query: User query string.
            k: Top-k chunks to retrieve (default 4).
            chunks: Optional candidate chunk pool.
        ...
        # 4. Handle empty retrieval - do NOT call generate_answer
        if not retrieved_chunks:
            return fallback_response

        # 3. Assemble context with source citations
        ...
        # Extract unique sources in order of appearance
        sources = []
...
# Alias for compatibility with varying casing conventions
